...
import { FiCheck, FiEdit2, FiPlus, FiSearch, FiShoppingCart, FiTrash2, FiX } from "react-icons/fi";
import { FooterNav } from "../components/FooterNav";
import { HeaderComponent } from "../components/HeaderComponent";
import SwipeShell from "../components/SwipeShell";
import { useItemContext } from "../hooks/useItemContext";
import { createShoppingItem, deleteShoppingItem, getErrorMessage, getShoppingHistory, getShoppingItems, purchaseShoppingItem, updateShoppingItem } from "../services/api";
...
export function ShoppingPage() {
  ...
  return <SwipeShell refresh={load}>
    ...
    <main className="mx-auto min-h-screen max-w-md space-y-3 px-4 pb-32 pt-[calc(var(--app-header-height,6rem)+1rem)] dark:text-white">
      ...
      {view === "history" && (loading ? <div className="py-12 text-center text-gray-500">Loading…</div> : !history.length ? <div className="rounded-2xl border border-dashed border-gray-300 p-8 text-center text-gray-500">No purchases recorded yet</div> : <div className="space-y-2">{history.map((item) => <article key={item.id} className="rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900"><div className="flex justify-between gap-3"><div><p className="font-semibold">{item.name}</p><p className="text-xs text-gray-500">Bought {item.purchasedQuantity} {item.unit}{item.source === "foodTracker" ? " · Added to Food Tracker" : ""}</p></div><time className="shrink-0 text-xs text-gray-500">{item.purchasedAt ? new Date(item.purchasedAt).toLocaleDateString() : ""}</time></div></article>)}</div>)}
    </main><FooterNav />
