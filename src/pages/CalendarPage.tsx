...
const hairStyleOptions: { value: HairStyleOption; label: string }[] = [
  ...
  { value: "stitch braids", label: "Stitch braids" },
];
...
export function CalendarPage() {
  ...
  const selectedDayTotal = getCalendarRevenue(selectedEntries);
  const formDayTotal = formData.clients.reduce(
  ...
  return (
    <SwipeShell toLeft="/settings" toRight="/tasks" refresh={fetchCalendarEntries}>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-lg bg-emerald-50 p-2 dark:bg-emerald-950/40">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-100">
                    Day total
                  </p>
                  <p className="mt-0.5 font-bold text-emerald-900 dark:text-emerald-50">
                    {formatCurrency(selectedDayTotal)}
                  </p>
                </div>
                <div className="rounded-lg bg-blue-50 p-2 dark:bg-blue-950/40">
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-blue-700 dark:text-blue-100">
                    Clients
                  </p>
                  <p className="mt-0.5 font-bold text-blue-900 dark:text-blue-50">
                    {selectedClients.length}
                  </p>
                </div>
              </div>
              {selectedClients.map((client) => {
                const clientPrice = client.price ?? 0;
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex shrink-0 flex-wrap justify-end gap-1">
                        {client.startTime && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-red-700 dark:bg-red-900/50 dark:text-red-100">
                            <FiClock className="h-3 w-3" />
                            {client.startTime}
                          </span>
                        )}
                        {clientPrice > 0 && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-emerald-700 dark:bg-red-900/50 dark:text-emerald-100">
      ...
      >
        <form className="max-h-[76vh] space-y-3 overflow-y-auto pr-1" onSubmit={handleSubmit}>
          <div>
          ...
          {formData.status === "booked" && (
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs dark:border-gray-700 dark:bg-gray-800/60">
                <div className="min-w-0">
                  <span className="font-semibold text-gray-500 dark:text-gray-300">
                    Day total
                  </span>
                  <span className="ml-2 font-bold text-emerald-700 dark:text-emerald-100">
                    {formatCurrency(formDayTotal)}
                  </span>
                </div>
                <div className="shrink-0 font-semibold text-blue-700 dark:text-blue-100">
                  {formData.clients.filter((client) => client.name.trim()).length} clients
                </div>
              </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          ...
                          updateClient(index, {
                            price:
                              event.target.value === "" || !Number.isFinite(price)
                                ? undefined
                                : price,
                          });
                        }}
